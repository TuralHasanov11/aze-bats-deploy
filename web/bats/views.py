...
from bats.models import Bat, BatAttributes, BatRedBook, Genus
...
def index(request):
    ...
    batPromoImage = SiteInfo.site_infos.bat_promo_image()

    genus = Genus.objects.all()
    return render(
        request, "bats/index.html", {"bats": bats, "genus": genus, "bat_promo_image": batPromoImage}
    )
...
class GalleryView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["bat_promo_image"] = SiteInfo.site_infos.bat_promo_image()
        return context
...
def detail(request, slug: str):
    bat = (
        Bat.objects.select_related("genus__family")
        .prefetch_related(
            "bat_images",
            Prefetch(
                "bat_attributes",
                queryset=BatAttributes.objects.filter(language=translation.get_language()),
            ),
            Prefetch(
                "bat_red_book",
                queryset=BatRedBook.objects.filter(language=translation.get_language()),
            ),
        )
        .get(slug=slug)
    )
